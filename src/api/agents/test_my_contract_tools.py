...
import os
import json
from api.agents.tools import orchestrate_exploit, monitor_account_activity
from api.web3_connection import get_web3_connection

# Load the ABI for the IntermediateVulnerableContract
abi_path = os.path.join(os.path.dirname(__file__), "../abi/IntermediateVulnerableContractABI.json")
with open(abi_path, 'r') as f:
    contract_abi = json.load(f)

# Define contract address and private key
contract_address = "0x410d6C06A80a26340B29b8eed39CCfCcF8A66049"  # IntermediateVulnerableContract
private_key = "a4af72ca5e6acefeea8593fd689892757bfd2df55bd7aff7b8d410cf4846a995"  # Make sure this is accurate for testing
...
# Test monitoring account activity
def test_monitor_account_activity():
    print(f"Monitoring account activity for contract: {contract_address}")
    try:
        events = monitor_account_activity(w3, contract_address, from_block='latest')
        print(f"Monitored events: {events}")
    except Exception as e:
        print(f"Error in monitoring account activity: {e}")

# Test orchestrating multi-step exploit
def test_orchestrate_exploit():
    print("Orchestrating multi-step exploit...")
    try:
        steps = [
            ("deposit", []),  # Step 1: Deposit Ether
            ("timeLimitedWithdrawal", []),  # Step 2: Time-dependent withdrawal
            ("privilegedAction", [])  # Step 3: Privileged action
        ]
        orchestrate_exploit(w3, private_key, contract_address, contract_abi, steps)
    except Exception as e:
        print(f"Error in orchestrating exploit: {e}")
...
def main():
    ...
    if balance_in_eth is not None and balance_in_eth > 0.1:
        # Test monitoring account activity
        test_monitor_account_activity()

        # Test orchestrating the multi-step exploit
